...
WEBHOOK_VERIFY_TOKEN=qatar_advisor_secure_token_2026
```
...
| `SUPABASE_URL` | رابط مشروع Supabase | Production, Preview |
| `SUPABASE_ANON_KEY` | مفتاح Supabase العام | Production, Preview |
| `UPSTASH_REDIS_REST_URL` | رابط Redis لـ Rate Limiting | Production, Preview |
| `UPSTASH_REDIS_REST_TOKEN` | مفتاح Redis | Production, Preview |
| `ADMIN_PASSWORD` | كلمة مرور لوحة الإدارة (لا قيمة افتراضية!) | Production |
| `CRON_SECRET` | مفتاح حماية Cron Jobs (PDPPL cleanup) | Production |
| `ADMIN_ORIGIN` | النطاق المسموح لـ CORS في Admin API | Production |
...
## الخطوة 9.5: التحقق من نقاط API الإضافية

تأكد أن نقاط الـ API التالية تعمل بشكل صحيح بعد النشر:

| Endpoint | الوصف |
|----------|-------|
| `/api/webhook` | نقطة استقبال رسائل WhatsApp (الأساسية) |
| `/api/admin` | لوحة الإدارة (محمية بـ `ADMIN_PASSWORD` + Rate Limiting) |
| `/api/cron-pdppl-cleanup` | حذف البيانات التلقائي كل 24 ساعة (محمي بـ `CRON_SECRET`) |
...
hub.mode=subscribe&\
hub.verify_token=qatar_advisor_secure_token_2026&\
...
| Rate Limit | 10 طلبات/دقيقة لكل IP (Upstash Redis sliding window) |

---

## الميزات الأمنية المُفعَّلة

- **Rate Limiting موزع** (Upstash Redis) مع fallback في الذاكرة
- **CSP + Security Headers** (مُعرَّفة في `vercel.json`)
- **Prompt Injection Sanitization** (`sanitizeInput`)
- **HMAC Webhook Signature Verification**
- **PDPPL Auto-Cleanup Cron** (الاحتفاظ بالبيانات لمدة 24 شهراً كحد أقصى)
- **Circuit Breaker** لمرونة التعامل مع أعطال الـ API
