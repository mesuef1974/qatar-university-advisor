...
class CircuitBreaker {
  ...
  recordSuccess() {
    ...
    if (this.state === STATES.HALF_OPEN) {
      ...
      if (this.successCount >= CONFIG.SUCCESS_THRESHOLD) {
        ...
        console.log(`${CONFIG.LOG_PREFIX} [${this.name}] ✅ Circuit CLOSED — Supabase عاد للعمل`);
      }
    }
  }
  ...
  recordFailure(error = '') {
    ...
    if (this.state === STATES.HALF_OPEN) {
      ...
      console.warn(`${CONFIG.LOG_PREFIX} [${this.name}] 🔄 اختبار فاشل → Circuit OPEN من جديد`);
      return;
    }

    if (this.state === STATES.CLOSED) {
      this.failureCount++;
      console.warn(`${CONFIG.LOG_PREFIX} [${this.name}] ⚠️  فشل ${this.failureCount}/${CONFIG.FAILURE_THRESHOLD}: ${error}`);

      if (this.failureCount >= CONFIG.FAILURE_THRESHOLD) {
        this._transitionTo(STATES.OPEN);
        console.error(`${CONFIG.LOG_PREFIX} [${this.name}] 🔴 Circuit OPEN — يتحول للـ In-Memory`);
      }
    }
  }
  ...
  _transitionTo(newState) {
    ...
    console.log(`${CONFIG.LOG_PREFIX} [${this.name}] ${prev} → ${newState}`);
  }
}
