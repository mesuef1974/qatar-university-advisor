 ...
 * ══════════════════════════════════════════════
 * سلسلة الاحتياط عند فشل Gemini API:
 *   1. Gemini API (عبر Circuit Breaker)
...
// ────────────────────────────────────────────────────────────────────────────
// Circuit Breaker مخصص لـ Gemini API
// ────────────────────────────────────────────────────────────────────────────
...
/**
 * محاولة الحصول على رد من Gemini عبر Circuit Breaker
 * @param {string} userMessage
 * @param {object} userProfile
 * @param {Array} conversationHistory
 * @returns {Promise<{text: string, suggestions: string[]}|null>}
 */
async function tryGemini(userMessage, _userProfile = {}, conversationHistory = []) {
  // إذا كانت الدائرة مفتوحة، لا نحاول أصلاً
  if (!geminiCircuit.canRequest()) {
    console.warn('[AI-Fallback] Gemini circuit OPEN — skipping Gemini');
    return null;
  }
  ...
  try {
    ...
    // Gemini returned null/empty — count as failure
    geminiCircuit.recordFailure('empty response from Gemini');
    return null;
  } catch (err) {
    geminiCircuit.recordFailure(err?.message || String(err));
    return null;
  }
}
...
async function tryStaticResponse(userMessage) {
  try {
    ...
    console.warn('[AI-Fallback] Static response lookup failed:', err?.message);
    return null;
  }
}
 ...
 * ═══════════════════════════════════════════
 * Gemini → Static Responses → Graceful Error
 *
...
async function getAIResponseWithFallback(userMessage, userProfile = {}, conversationHistory = []) {
  // ── المستوى 1: Gemini API ──────────────────────────────
  const geminiResult = await tryGemini(userMessage, userProfile, conversationHistory);
  if (geminiResult) {
    console.log('[AI-Fallback] Level 1: Gemini response OK');
    return { ...geminiResult, fallbackLevel: FALLBACK_LEVELS.GEMINI };
  }

  // ── المستوى 2: الردود الثابتة ──────────────────────────
  console.warn('[AI-Fallback] Gemini failed — trying static responses');
  const staticResult = await tryStaticResponse(userMessage);
  if (staticResult) {
    console.log('[AI-Fallback] Level 2: Static response matched');
    return { ...staticResult, fallbackLevel: FALLBACK_LEVELS.STATIC };
  }

  // ── المستوى 3: رسالة خطأ ودية ──────────────────────────
  console.error('[AI-Fallback] All fallbacks exhausted — returning graceful error');
  return { ...GRACEFUL_ERROR_MESSAGE, fallbackLevel: FALLBACK_LEVELS.GRACEFUL_ERROR };
}

/**
 * حالة صحة Gemini Circuit Breaker للمراقبة
 */
function getGeminiCircuitStatus() {
  return geminiCircuit.getStatus();
}
