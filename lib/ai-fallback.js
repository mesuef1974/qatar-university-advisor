...
async function tryGemini(userMessage, _userProfile = {}, conversationHistory = []) {
  ...
  if (!geminiCircuit.canRequest()) {
    logger.warn('[AI-Fallback] Gemini circuit OPEN — skipping Gemini');
    return null;
  }
...
async function getAIResponseWithFallback(userMessage, userProfile = {}, conversationHistory = []) {
  ...
  if (geminiResult) {
    logger.info('[AI-Fallback] Level 1: Gemini response OK');
    return { ...geminiResult, fallbackLevel: FALLBACK_LEVELS.GEMINI };
  }

  // ── المستوى 2: الردود الثابتة ──────────────────────────
  logger.warn('[AI-Fallback] Gemini failed — trying static responses');
