...
export function addSecurityHeaders(res) {
  ...
  res.setHeader(
    ...
    [
      "default-src 'self'",
      // No 'unsafe-inline' or 'unsafe-eval' — Vite production builds use hashed modules
      "script-src 'self' https://cdn.jsdelivr.net",
      // 'unsafe-inline' kept in style-src: React applies inline styles at runtime
      // (e.g. CSS-in-JS, dynamic style props) which cannot be replaced by hashes
      "style-src 'self' 'unsafe-inline' https://fonts.googleapis.com",
      "font-src 'self' https://fonts.gstatic.com",
      "img-src 'self' data: https:",
      "connect-src 'self' https://*.supabase.co wss://*.supabase.co https://api.anthropic.com https://generativelanguage.googleapis.com https://graph.facebook.com",
  ...
  // Cross-Origin isolation headers (matches vercel.json)
