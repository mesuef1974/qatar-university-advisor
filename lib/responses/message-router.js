// DEPRECATED — use findResponse.ts instead
import { qatariDialect } from '../dialect-map.js';

/**
 * Message Router — Qatar University Advisor
 * ══════════════════════════════════════════
 * DEPRECATED: هذا الملف مكرر من findResponse.ts — استخدم findResponse.ts بدلاً منه.
 * تم الإبقاء عليه مؤقتاً لتجنب كسر الاستيرادات الموجودة.
 *
...
export function findResponse(text) {
  ...
  // S-010: ترجمة العامية القطرية → الفصحى (from shared dialect-map.ts)
  let normalizedQ = q;
