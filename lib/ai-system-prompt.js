 ...
 *
 * الهدف: Gemini يجيب من البيانات الفعلية وليس من معرفته العامة.
 *
...
function buildDatabaseSummary() {
  try {
    ...
    const uniLines = Object.entries(unis).map(([id, u]) => {
      ...
      } else if (deadlines.fallSemester2025) {
        deadlineInfo = `فتح: ${deadlines.fallSemester2025.open} — إغلاق: ${deadlines.fallSemester2025.close}`;
      }
...
/**
 * يبني System Prompt الكامل لـ Gemini مع بيانات الجامعات الحقيقية
 * @param {string} [nationality] - 'qatari' | 'non_qatari' | undefined
 * @returns {string} system prompt غني بالبيانات
 */
export function buildSystemPrompt(nationality) {
...
═══════════════════════════════════════════════════════
1. أنت "مُرشِدي" — لا تذكر أنك Gemini أو Google أو أي نموذج ذكاء اصطناعي
...
═══════════════════════════════════════════════════════
** استخدم البيانات التالية كمصدر حقيقة أولي. لا تختلق أرقاماً. **
** إذا لم تجد المعلومة في البيانات → قل "أنصحك بزيارة الموقع الرسمي للجامعة" **
