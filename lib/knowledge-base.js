 ...
 * ═══════════════════════════════════════════════════
 * نظام الذاكرة الذاتية: يحفظ ردود Claude الجيدة ويُعيد استخدامها
 *
 * الفلسفة:
 *   1. ابحث أولاً في الكاش → أسرع + أرخص + أكثر اتساقاً
 *   2. إذا لم تجد → اسأل Claude → احفظ الرد تلقائياً
 ...
 * يُستدعى تلقائياً بعد كل رد من Claude
 * @param {string} question - سؤال المستخدم الأصلي
 * @param {string} answer - إجابة Claude
 ...
 * 3. إذا لم يجد → يُعيد null (ليسأل Claude)
 *
 * @param {string} question - سؤال المستخدم
...
/**
 * توليد Embedding لنص باستخدام Claude
 * @param {string} text
...
const SEED_FAQ = [
  ...
  },
  {
    ...
    answer: 'تتراوح رسوم جامعة كارنيجي ميلون قطر (CMU-Q) بين 55,000 و65,000 ريال قطري سنوياً (نحو 15,000–18,000 دولار) وفق التخصص. ويُتاح للطلاب المتميزين التقدم للمنح الجزئية أو الكاملة. تعتبر CMU-Q من أغلى الجامعات في المدينة التعليمية، غير أن شهاداتها معادلة لشهادات الكامبس الأمريكي.',
  ...
  },
  {
    ...
    answer: 'جامعة قطر (QU) هي الجامعة الوطنية الحكومية الكبرى وتقبل آلاف الطلاب سنوياً بأسعار معقولة، بينما جامعة حمد بن خليفة (HBKU) مؤسسة بحثية نخبوية في المدينة التعليمية تركز على الدراسات العليا والبحث المتخصص. HBKU أصغر وأكثر تخصصاً وترتبط ارتباطاً وثيقاً بمؤسسة قطر.',
  ...
  },
  {
    ...
    answer: 'تكاد تكون الدراسة في جامعة قطر مجانية للمواطنين القطريين، إذ تتحمل الدولة رسوم التعليم بالكامل. علاوةً على ذلك، يتلقى الطالب القطري مكافأة شهرية تُقدَّر بنحو 3,000 ريال طوال سنوات الدراسة. يُضاف إلى ذلك إمكانية الحصول على سكن جامعي مدعوم وخدمات طلابية متكاملة.',
 ...
 * - يولّد embedding لكل سؤال باستخدام Claude (text-embedding-004)
...
async function seedKnowledgeCache({ force: _force = false, withEmbeddings = true } = {}) {
  ...
  for (const faq of SEED_FAQ) {
    try {
      ...
      // تأخير بسيط لتجنب rate limiting على Claude API
      if (withEmbeddings) {
