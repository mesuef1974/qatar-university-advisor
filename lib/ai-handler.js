...
3. التخصيص: إذا عرفت جنسية المتحدث، خصص ردك له:
   — قطري: أظهر كل المنح (البعثة الأميرية + طموح + الشركات + HBKU)
   — غير قطري: أظهر فقط المنح المتاحة لغير القطريين (HBKU + المدينة التعليمية). لا تذكر البعثة الأميرية أو برنامج طموح أو الابتعاث الداخلي لأنها حصرية للقطريين
   — إذا سأل "منح لغير القطريين": أجب فقط بمنح HBKU والمدينة التعليمية، ووضّح أن البعثة الأميرية وطموح خاصة بالقطريين
...
async function callGemini(userMessage, conversationHistory = []) {
  ...
  const normalizedMessage = normalizeDialect(sanitizedMessage);
